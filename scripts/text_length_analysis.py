...
class TextLengthAnalyzer:
    ...
    def calculate_text_length_distribution(self, messages: List[Any]) -> Dict[str, int]:
        """计算文本长度分布"""
    ...
    def get_text_length_stats(self, messages: List[Any]) -> Dict[str, float]:
        """计算文本长度统计信息"""
        lengths = []
        null_count = 0
        excluded_count = 0
    ...
    def get_available_chats(self) -> List[Tuple[str, str, int]]:
        ...
        try:
            # 查询方式
            query = Messages.select(Messages.chat_id).where(
                (Messages.is_emoji != 1) & 
                (Messages.is_picid != 1) & 
    ...
    def get_top_longest_messages(self, messages: List[Any], top_n: int = 10) -> List[Tuple[str, int, str, str]]:
        """获取最长的消息"""
        message_data = []
    ...
    def _display_results(self, chat_id: Optional[str], start_time: Optional[float],
                        end_time: Optional[float], messages: List[Any], 

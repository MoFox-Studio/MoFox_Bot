...
class ConfigManager:
    ...
    def ensure_env_file():
        ...
        try:
            ...
            logger.error(f"创建.env文件时发生未知错误: {e}")
            sys.exit(1)

    @staticmethod
    def verify_env_file_integrity() -> bool:
        """验证.env文件完整性"""
        env_file = Path(".env")
        ...
        try:
            ...
            if file_size == 0:
                logger.error(".env文件为空")
                return False
            if file_size > MAX_ENV_FILE_SIZE:
                logger.error(f".env文件大小异常: {file_size}字节，超过限制{MAX_ENV_FILE_SIZE}字节")
                return False
            ...
            logger.error(f"验证.env文件完整性时发生未知错误: {e}")
            return False
...
class EULAManager:
    ...
    async def check_eula():
        ...
        for attempt in range(1, MAX_EULA_CHECK_ATTEMPTS + 1):
            try:
                await asyncio.sleep(EULA_CHECK_INTERVAL)

                if not ConfigManager.safe_load_dotenv():
                    confirm_logger.warning(f"重新加载环境变量失败 (尝试 {attempt}/{MAX_EULA_CHECK_ATTEMPTS})")
                    continue
                ...
                if attempt % 5 == 0:
                    confirm_logger.critical(
                        f"请修改 .env 文件中的 EULA_CONFIRMED=true (尝试 {attempt}/{MAX_EULA_CHECK_ATTEMPTS})"
                    )
                ...
                if attempt >= MAX_EULA_CHECK_ATTEMPTS:
...
class TaskManager:
    ...
    @staticmethod
    async def cancel_pending_tasks(loop, timeout: float = SHUTDOWN_TIMEOUT) -> bool:
        """取消所有待处理的任务"""
        try:
            ...
            try:
                ...
                logger.info(f"任务取消完成: {cancelled_count}个已取消, {exception_count}个有异常")
                return True
                ...
                if still_running:
                    logger.warning(f"{len(still_running)}个任务仍在运行")
                return False
        except Exception as e:
            logger.error(f"取消待处理任务时发生异常: {e}")
            return False

    @staticmethod
    async def stop_async_tasks() -> bool:
        """停止所有异步任务"""
        try:
...
class ShutdownManager:
    ...
    @staticmethod
    async def graceful_shutdown(loop=None) -> bool:
        """优雅关闭程序"""
        ...
        try:
            # 停止 WebUI 开发服务
            webui_stopped = True  # 默认认为成功
            try:
                webui_stopped = await WebUIManager.stop_dev_server()
            except NameError:
                logger.debug("WebUIManager未定义，跳过WebUI停止")
            except Exception as e:
            ...
            status_msg = "成功" if success else "部分操作未成功"
            logger.info(f"麦麦优雅关闭{status_msg}，耗时: {shutdown_time:.2f}秒")
            return success
    ...
    def show(cls):
        ...
        if not cls._initialized:
            try:
                ...
                logger.warning(f"初始化colorama失败: {e}")
                return
        ...
        try:
            rainbow_text = "".join(rainbow_colors[i % len(rainbow_colors)] + char 
                                 for i, char in enumerate(text))
            logger.info(rainbow_text)
        except Exception as e:
            logger.warning(f"显示彩蛋失败: {e}")
...
class WebUIManager:
    ...
    @staticmethod
    def _resolve_webui_dir() -> Optional[Path]:
        """解析 webui 目录路径"""
        try:
            ...
            candidates = [
                ...
                script_dir.parent / "webui", # 上级目录/webui（兼容）
            ]
    ...
    async def start_dev_server(timeout: float = 60.0) -> bool:
        ...
        try:
            ...
            logger.info(f"正在启动 WebUI 开发服务器: npm run dev (目录: {webui_dir})")
            ...
            while time.time() - start_time < timeout:
                if proc.returncode is not None:
                    status = "异常退出" if proc.returncode != 0 else "正常退出"
                    logger.error(f"WebUI 进程{status}，退出码: {proc.returncode}")
                    break
                ...
                try:
                    ...
                    if any(keyword in lower_text for keyword in success_keywords):
    ...
    async def stop_dev_server(timeout: float = 5.0) -> bool:
        ...
        try:
            ...
            if WebUIManager._drain_task and not WebUIManager._drain_task.done():
                ...
                try:
                    await asyncio.wait_for(WebUIManager._drain_task, timeout=1.0)
            ...
            logger.error(f"停止 WebUI 开发服务器时出错: {e}")
            return False
        finally:
...
class MaiBotMain:
    ...
    def create_main_system(self):
        ...
        try:
            ...
            logger.error(f"导入MainSystem失败: {e}")
            raise
    ...
    async with create_event_loop_context() as loop:
        try:
            ...
            if webui_ok:
                ...
                logger.warning("WebUI 启动失败，继续下一步骤")
            ...
            user_input_task = asyncio.create_task(wait_for_user_input())
                ...
                [main_task, user_input_task], 
                return_when=asyncio.FIRST_COMPLETED
            )

            # 修复变量名错误：user_task -> user_input_task
            if user_input_task in done and main_task not in done:

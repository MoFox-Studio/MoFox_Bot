    ...
    async def _async_cleanup(self):
        ...
        if cleanup_tasks:
            ...
            try:
                ...
                # 记录结果
                for i, (name, task) in enumerate(zip(task_names, tasks)):
                    if isinstance(task.exception() if hasattr(task, 'exception') else None, Exception):
                        logger.error(f"停止 {name} 时出错")
                    else:
...
""")
    ...
    async def _init_components(self):
        ...
        # 表情管理器
        manager_init_tasks.append(asyncio.create_task(self._safe_init(
            "表情包管理器", 
            lambda: get_emoji_manager().initialize()
        )))
        ...
        manager_init_tasks.append(asyncio.create_task(self._safe_init(
            "聊天管理器", 
            get_chat_manager()._initialize
        )))
        
        # 等待所有管理器初始化完成
        await asyncio.gather(*manager_init_tasks, return_exceptions=True)
    ...
    async def _init_planning_components(self):
        """初始化计划相关组件"""
        # 初始化月度计划管理器
    ...
    async def _safe_init(self, component_name: str, init_func):
        """安全初始化组件，捕获异常"""
        async def wrapper():
            try:
                result = init_func()
